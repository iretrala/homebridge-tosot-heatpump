...
class Device {
    ...
    constructor(options) {
        ...
        this.options = {
            ...
            onConnected: options.onConnected || function () {
                console.log("[TosotHeatpump]: connected to host %s", options.host);
            },
            onError: options.onError || function () {
                console.log("[TosotHeatpump]: error occurred %s", options.host, arguments);
            },
            onDisconnected: options.onDisconnected || function () {
                console.log("[TosotHeatpump]: disconnected from host %s", options.host, arguments);
            },
        ...
        console.log("[TosotHeatpump]: init deviceFactory on host %s [server port %s]", that.options.host, that.options.port);
        ...
        try {
            this.socket.bind(port, "0.0.0.0", () => {
                ...
                this.socket.send(message, 0, message.length, that.options.defaultPort, address, error => {
                    if (error) {
                        console.log("[TosotHeatpump]: _connectToDevice socket error %s", address, error);
                    }
                });
            });
        } catch (err) {
            console.log("[TosotHeatpump]: _connectToDevice error - port %d %s", port, err);
            const timeout = 5
    ...
    _setDevice(id, name, address, port) {
        ...
        that.device.props = {};
        console.log('[TosotHeatpump] New device added', that.device);
    }
    ...
    _confirmBinding(id, key) {
        ...
        that.device.key = key;
        console.log('[TosotHeatpump] device is bound: %s - %s', that.device.name, that.device.key);
    }
    ...
    _handleResponse(msg, rinfo) {
        ...
        try {
            ...
            if (pack.t === 'dev') {
                console.log('[TosotHeatpump] response to handshake:{}', rinfo);
            ...
            console.log("[TosotHeatpump]: _handleResponse error", msg, rinfo, err);
    ...
    _sendRequest(message, address = this.device.address, port = this.device.port) {
        ...
        try {
            this.socket.send(serializedRequest, 0, serializedRequest.length, port, address, error => {
                if (error) {
                    console.log("[TosotHeatpump]: _sendRequest socket error", error);
                }
            });
        } catch (e) {
            console.log("[TosotHeatpump]: _sendRequest error", e);
        }
    };
